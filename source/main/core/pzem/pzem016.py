    ...
    def launcher(self):
        ...
        while True:
            ...
            if data_count > 0:
                ...
                voltage = SensorUtil.hex_to_dec(hex_data[6:10])       # parse voltage data 2B
                current = SensorUtil.hex_to_dec(hex_data[10:18])      # parse current data 4B
                power = SensorUtil.hex_to_dec(hex_data[18:26])        # parse power data 4B
                energy = SensorUtil.hex_to_dec(hex_data[26:34])       # parse energy data 4B
                frequency = SensorUtil.hex_to_dec(hex_data[34:38])    # parse frequency data 2B
                powerfactor = SensorUtil.hex_to_dec(hex_data[38:42])  # parse powerfactor data 2B
                alarmstatus = SensorUtil.hex_to_dec(hex_data[42:46])  # parse alarmstatus data 2B
